...
import { fetchFeed, findNewEntries, formatPost, getPlaceholders } from "./rssUtil.js";
...
export default (client) => {
    ...
    cron.schedule("*/10 * * * *", async () => {
        ...
        servers.forEach(async ({ gid, feeds }) => {
            // Loop their feeds
            Object.values(feeds).forEach(
                async ({ fid, cid, url, oldData, errorCount, format, filter }) => {
                    // Catch any errors
                    try {
                        // Fetch the channel
                        let channel;
